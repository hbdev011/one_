class GpOutput < ActiveRecord::Base

	def self.get_outputs(system_id)
	...
	def self.add_last(system_id, rows=1)
		...
		nummer == 0 ? val = 2000 : val = last.value

		(1..rows).each do
			self.new(:intnummer => nummer+=1, :value => val+=1, :system_id => system_id, :invert => 0, :o_type => 1, :mode => 3).save
		end
		return nummer
	end

	def self.get_output_names(system_id)
